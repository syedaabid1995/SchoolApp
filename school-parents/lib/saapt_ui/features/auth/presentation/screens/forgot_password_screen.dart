...
class _SaaptForgotPasswordScreenState
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(
        () => _error = parentApiError(error, 'Unable to reset password'),
      );
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final subtitle = switch (_step) {
      _ForgotPasswordStep.email => 'Enter your registered parent email.',
      _ForgotPasswordStep.otp => 'Check your email for the 6-digit OTP.',
      _ForgotPasswordStep.password => 'Create a new parent login password.',
    };
