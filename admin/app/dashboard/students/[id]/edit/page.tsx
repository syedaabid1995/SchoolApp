...
export default function EditStudentPage() {
  ...
  const feeMastersQuery = useQuery({
    ...
    enabled: canEditStudent && Boolean(form.academicSessionId),
  });
  ...
  const siblingsQuery = useQuery({
    ...
    enabled: canEditStudent && Boolean(siblingFilters.classId && siblingFilters.sectionId),
  });
  ...
  useEffect(() => {
    if (!student) return;
    const siblingSummaries: Record<string, SelectedSiblingSummary> = {};
    (student.siblings ?? []).forEach((item) => {
      const sibling = item.sibling;
      if (!sibling?.id) return;
      siblingSummaries[sibling.id] = {
        id: sibling.id,
        name: sibling.fullName ?? 'Sibling',
        admissionNo: sibling.admissionNo,
      };
    });
    const [firstName, ...lastNameParts] = (student.fullName ?? `${student.firstName} ${student.lastName}`.trim()).split(/\s+/);
    ...
    setForm({
      ...
      firstName: firstName ?? '',
      lastName: lastNameParts.join(' ') || student.lastName || '',
      ...
      facePhotoUrls: student.faceProfile?.samples?.map((sample) => sample.imageUrl).filter((url): url is string => Boolean(url)) ?? [],
      ...
      createParentLogin: false,
      parentLoginSource: 'father',
      parentLoginFirstName: linkedParent?.firstName ?? '',
      parentLoginLastName: linkedParent?.lastName ?? '',
      parentLoginPhone: linkedParent?.phone ?? student.parentPhone ?? student.fatherPhone ?? '',
      parentLoginEmail: linkedParent?.email ?? student.parentEmail ?? '',
      parentLoginSendVia: 'SMS',
      presentAddress: student.presentAddress ?? student.addressLine1 ?? '',
      permanentAddress: student.permanentAddress ?? student.addressLine2 ?? '',
      siblingIds: Object.keys(siblingSummaries),
    });
    setSelectedSiblingSummaries(siblingSummaries);
    setSameAddress(Boolean(student.presentAddress && student.presentAddress === student.permanentAddress));
  }, [student]);
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: ({ student, parentLogin, parentLoginError }) => {
      ...
      if (student.faceRegistration?.success) {
        ...
        notify.error('Face registration failed', student.faceRegistration.error ?? 'Student was updated, but AI attendance face registration failed.');
      }
      if (parentLogin) {
  ...
  const toggleFeeDiscount = (discount: FeeDiscount) => {
    ...
    if (isExpired(discount.expiryDate ?? discount.validTo)) {
      notify.error('Discount expired', 'Expired discounts cannot be selected for this student.');
      return;
    }
    setForm((prev) => {
  ...
  if (isSessionLoading || !session?.role || studentQuery.isLoading) {
    return <FullPageLoader label="Loading student edit form..." />;
  }
  if (!canEditStudent) {
    return (
      <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-sm font-semibold text-amber-800">
        Student editing is not enabled for your role. Ask a School Admin to update Role Permissions.
      </section>
    );
  }
  if (!student) {
        ...
        <div className="grid gap-5 xl:grid-cols-[1.35fr_0.65fr]">
          <div className="space-y-5">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">Create or reuse a parent portal login and link it to this student.</p>
                </div>
                <label className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-700">
                  <input
                    type="checkbox"
            ...
            {currentStep === 'review' ? <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="mb-4 text-lg font-bold text-slate-950">Review Updates</h2>
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                  ...
                  <p className="text-sm text-slate-600">Parent login: <span className="font-semibold text-slate-900">{form.createParentLogin ? 'Will be created or linked' : 'Not enabled'}</span></p>
                </div>
                <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
