...
import { approveExportRequest, getComplianceJobHistory, getExportRequestById, rejectExportRequest } from '../../../../../../services/compliance.service';
...
export default function ExportReviewPage() {
  ...
  const { data: request, isLoading } = useQuery({ queryKey: ['compliance-export', id], queryFn: () => getExportRequestById(id) });
  const { data: history = [] } = useQuery({ queryKey: ['compliance-history', id], queryFn: () => getComplianceJobHistory(id) });

  const refresh = () => {
    queryClient.invalidateQueries({ queryKey: ['compliance-export', id] });
    queryClient.invalidateQueries({ queryKey: ['compliance-history', id] });
  };

  const approveMutation = useMutation({
    mutationFn: () => approveExportRequest(id, { note }),
    onSuccess: () => {
      notify.success('Approved', 'Export request approved. Execution remains controlled by the backend export workflow.');
      refresh();
    },
    onError: (error: any) => notify.error('Approval failed', error?.response?.data?.error?.message || 'Unable to approve export request.'),
  });

  const rejectMutation = useMutation({
    mutationFn: () => rejectExportRequest(id, { reason }),
    onSuccess: () => {
      notify.success('Rejected', 'Export request rejected.');
      setReason('');
      refresh();
    },
    onError: (error: any) => notify.error('Rejection failed', error?.response?.data?.error?.message || 'Unable to reject export request.'),
  });
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-10">
      <div className="mx-auto max-w-6xl pr-6">
        <PageHeader title="Review Export Request" subtitle={id} actions={<Link className="rounded-md border bg-white px-3 py-2 text-sm font-semibold" href="/dashboard/compliance">Back</Link>} />
