...
const resolveAcademicSessionId = async (schoolId: string, requested?: string | null) => {
  if (requested) {
    const found = await FeeReportingRepository.academicYear.findFirst({ where: { id: requested, schoolId }, select: { id: true } });
    if (!found) throw new HttpError(404, 'Academic session not found');
    return requested;
  }

  const active = await FeeReportingRepository.academicYear.findFirst({
  ...
  const latest = await FeeReportingRepository.academicYear.findFirst({
    where: { schoolId },
    orderBy: { startDate: 'desc' },
    select: { id: true },
  });
...
const ensureFeeDefaults = async (schoolId: string, academicSessionId: string) => {
  const [particularCount, typeCount] = await Promise.all([
    FeeReportingRepository.feeParticular.count({ where: { schoolId, academicSessionId } }),
    FeeReportingRepository.feeType.count({ where: { schoolId, academicSessionId } }),
  ]);

  if (!particularCount) {
    await FeeReportingRepository.feeParticular.createMany({
      skipDuplicates: true,
  ...
  if (!typeCount) {
    await FeeReportingRepository.feeType.createMany({
      skipDuplicates: true,
...
const assertClass = async (schoolId: string, classId: string) => {
  const found = await FeeReportingRepository.class.findFirst({ where: { id: classId, schoolId }, select: { id: true } });
  if (!found) throw new HttpError(404, 'Class not found');
};

const assertSection = async (schoolId: string, sectionId?: string | null) => {
  if (!sectionId) return;
  const found = await FeeReportingRepository.section.findFirst({ where: { id: sectionId, schoolId }, select: { id: true } });
...
const assertStudent = async (schoolId: string, studentId: string, academicSessionId?: string) => {
  const found = await FeeReportingRepository.student.findFirst({
...
const assertFeeType = async (schoolId: string, academicSessionId: string, feeTypeId: string) => {
  const found = await FeeReportingRepository.feeType.findFirst({
...
const assertFeeGroup = async (schoolId: string, academicSessionId: string, feeGroupId: string) => {
  const found = await FeeReportingRepository.feeGroup.findFirst({
...
const assertFeeMaster = async (schoolId: string, academicSessionId: string, feeMasterId: string) => {
  const found = await FeeReportingRepository.feeMaster.findFirst({
...
const assertStudentCategory = async (schoolId: string, categoryId: string) => {
  const found = await FeeReportingRepository.studentCategory.findFirst({ where: { id: categoryId, schoolId }, select: { id: true } });
  if (!found) throw new HttpError(404, 'Student category not found');
};

const assertStudentGroup = async (schoolId: string, groupId: string) => {
  const found = await FeeReportingRepository.studentGroup.findFirst({ where: { id: groupId, schoolId }, select: { id: true } });
...
const assertTransportRoute = async (schoolId: string, transportRouteId: string) => {
  const found = await FeeReportingRepository.transportRoute.findFirst({ where: { id: transportRouteId, schoolId }, select: { id: true } });
...
const assertParticulars = async (schoolId: string, academicSessionId: string, ids: string[]) => {
  ...
  const found = await FeeReportingRepository.feeParticular.findMany({
...
const assertNoDuplicateActiveDiscount = async (scope: FeeTenantScope, payload: NormalizedDiscountPayload, excludeId?: string) => {
  ...
  const duplicate = await FeeReportingRepository.feeDiscount.findFirst({
    where: {
...
const assertDiscountNotLockedByPayment = async (scope: FeeTenantScope, discount: FeeDiscount) => {
  const paidCount = await FeeReportingRepository.feeInvoice.count({ where: buildDiscountInvoiceWhere(scope, discount) });
  if (paidCount > 0) {
...
const assertDiscountDoesNotExceedCurrentPayable = async (scope: FeeTenantScope, payload: NormalizedDiscountPayload) => {
  ...
  const payable = toDecimal(
    (
      await FeeReportingRepository.feeInvoice.aggregate({
        where: {
          ...tenantScopeOnly(scope),
          studentId: payload.studentId,
          deletedAt: null,
...
const assertAssignmentReferences = async (scope: FeeTenantScope, payload: z.infer<typeof assignmentSchema>, targetType: FeeAssignmentTarget) => {
  ...
  if (targetType === 'STUDENT') {
    ...
    const found = await FeeReportingRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: ids } }), select: { id: true } });
...
const resolveAssignmentStudents = async (
  ...
  if (targetType === 'STUDENT') {
    return FeeReportingRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: studentIds } }), include, orderBy: { fullName: 'asc' } });
  }
  if (targetType === 'TRANSPORT_ROUTE') {
    const rows = await FeeReportingRepository.studentTransportAssignment.findMany({
      where: { schoolId: scope.schoolId, routeId: payload.transportRouteId ?? '', active: true },
      select: { studentId: true },
    });
    return FeeReportingRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: rows.map((row) => row.studentId) } }), include, orderBy: { fullName: 'asc' } });
  }
  return FeeReportingRepository.student.findMany({
