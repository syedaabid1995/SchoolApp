...
import { z } from 'zod';
import { FeeAssignmentRepository } from '../../repositories/assignment.repository';
...
const resolveAcademicSessionId = async (schoolId: string, requested?: string | null) => {
  if (requested) {
    const found = await FeeAssignmentRepository.academicYear.findFirst({ where: { id: requested, schoolId }, select: { id: true } });
    if (!found) throw new HttpError(404, 'Academic session not found');
    return requested;
  }

  const active = await FeeAssignmentRepository.academicYear.findFirst({
  ...
  const latest = await FeeAssignmentRepository.academicYear.findFirst({
    where: { schoolId },
    orderBy: { startDate: 'desc' },
    select: { id: true },
  });
...
const ensureFeeDefaults = async (schoolId: string, academicSessionId: string) => {
  const [particularCount, typeCount] = await Promise.all([
    FeeAssignmentRepository.feeParticular.count({ where: { schoolId, academicSessionId } }),
    FeeAssignmentRepository.feeType.count({ where: { schoolId, academicSessionId } }),
  ]);

  if (!particularCount) {
    await FeeAssignmentRepository.feeParticular.createMany({
      skipDuplicates: true,
  ...
  if (!typeCount) {
    await FeeAssignmentRepository.feeType.createMany({
      skipDuplicates: true,
...
const assertClass = async (schoolId: string, classId: string) => {
  const found = await FeeAssignmentRepository.class.findFirst({ where: { id: classId, schoolId }, select: { id: true } });
  if (!found) throw new HttpError(404, 'Class not found');
};

const assertSection = async (schoolId: string, sectionId?: string | null) => {
  if (!sectionId) return;
  const found = await FeeAssignmentRepository.section.findFirst({ where: { id: sectionId, schoolId }, select: { id: true } });
...
const assertStudent = async (schoolId: string, studentId: string, academicSessionId?: string) => {
  const found = await FeeAssignmentRepository.student.findFirst({
...
const assertFeeType = async (schoolId: string, academicSessionId: string, feeTypeId: string) => {
  const found = await FeeAssignmentRepository.feeType.findFirst({
...
const assertFeeGroup = async (schoolId: string, academicSessionId: string, feeGroupId: string) => {
  const found = await FeeAssignmentRepository.feeGroup.findFirst({
...
const assertFeeMaster = async (schoolId: string, academicSessionId: string, feeMasterId: string) => {
  const found = await FeeAssignmentRepository.feeMaster.findFirst({
...
const assertStudentCategory = async (schoolId: string, categoryId: string) => {
  const found = await FeeAssignmentRepository.studentCategory.findFirst({ where: { id: categoryId, schoolId }, select: { id: true } });
  if (!found) throw new HttpError(404, 'Student category not found');
};

const assertStudentGroup = async (schoolId: string, groupId: string) => {
  const found = await FeeAssignmentRepository.studentGroup.findFirst({ where: { id: groupId, schoolId }, select: { id: true } });
...
const assertTransportRoute = async (schoolId: string, transportRouteId: string) => {
  const found = await FeeAssignmentRepository.transportRoute.findFirst({ where: { id: transportRouteId, schoolId }, select: { id: true } });
...
const assertParticulars = async (schoolId: string, academicSessionId: string, ids: string[]) => {
  ...
  const found = await FeeAssignmentRepository.feeParticular.findMany({
...
const assertNoDuplicateActiveDiscount = async (scope: FeeTenantScope, payload: NormalizedDiscountPayload, excludeId?: string) => {
  ...
  const duplicate = await FeeAssignmentRepository.feeDiscount.findFirst({
    where: {
...
const assertDiscountNotLockedByPayment = async (scope: FeeTenantScope, discount: FeeDiscount) => {
  const paidCount = await FeeAssignmentRepository.feeInvoice.count({ where: buildDiscountInvoiceWhere(scope, discount) });
  if (paidCount > 0) {
...
const assertDiscountDoesNotExceedCurrentPayable = async (scope: FeeTenantScope, payload: NormalizedDiscountPayload) => {
  ...
  const payable = toDecimal(
    (
      await FeeAssignmentRepository.feeInvoice.aggregate({
        where: {
          ...tenantScopeOnly(scope),
          studentId: payload.studentId,
          deletedAt: null,
...
const assertAssignmentReferences = async (scope: FeeTenantScope, payload: z.infer<typeof assignmentSchema>, targetType: FeeAssignmentTarget) => {
  ...
  if (targetType === 'STUDENT') {
    ...
    const found = await FeeAssignmentRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: ids } }), select: { id: true } });
...
const resolveAssignmentStudents = async (
  ...
  if (targetType === 'STUDENT') {
    return FeeAssignmentRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: studentIds } }), include, orderBy: { fullName: 'asc' } });
  }
  if (targetType === 'TRANSPORT_ROUTE') {
    const rows = await FeeAssignmentRepository.studentTransportAssignment.findMany({
      where: { schoolId: scope.schoolId, routeId: payload.transportRouteId ?? '', active: true },
      select: { studentId: true },
    });
    return FeeAssignmentRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: rows.map((row) => row.studentId) } }), include, orderBy: { fullName: 'asc' } });
  }
  return FeeAssignmentRepository.student.findMany({
...
export const listFeeAssignments = async (req: Request, res: Response) => {
  ...
  const [items, total, activeAssignments] = await Promise.all([
    FeeAssignmentRepository.studentFeeAssignment.findMany({
      where,
    ...
    FeeAssignmentRepository.studentFeeAssignment.count({ where }),
    FeeAssignmentRepository.studentFeeAssignment.findMany({
  ...
  const students = await FeeAssignmentRepository.student.findMany({
  ...
  const routeRows = await FeeAssignmentRepository.studentTransportAssignment.findMany({
...
export const assignStudentFees = async (req: Request, res: Response) => {
  ...
  const structure = await FeeAssignmentRepository.feeStructure.findFirst({ where: { id: payload.feeStructureId, ...tenantScopeOnly(scope), deletedAt: null, status: 'ACTIVE' } });
  ...
  for (const studentId of studentIds) {
    const duplicate = await FeeAssignmentRepository.studentFeeAssignment.findFirst({
  ...
  const created = await FeeAssignmentRepository.$transaction(async (tx) =>
    Promise.all(
  ...
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'STUDENT_FEE_ASSIGNMENT', entityId: payload.feeStructureId, action: 'CREATE', afterState: { assignmentIds: created.map((item) => item.id), assignedStudentIds: students.map((student) => student.id), targetType } });
...
export const updateFeeAssignment = async (req: Request, res: Response) => {
  const payload = assignmentSchema.parse(req.body);
  const scope = await resolveScope(req, payload);
  const existing = await FeeAssignmentRepository.studentFeeAssignment.findFirst({ where: { id: uuidParam(req), ...tenantScopeOnly(scope), deletedAt: null } });
  ...
  if (duplicate) throw new HttpError(409, 'Duplicate active assignment exists for the same target, structure, and academic session');
  const item = await FeeAssignmentRepository.studentFeeAssignment.update({
    where: { id: existing.id },
    data: assignmentScalarUpdateData(payload, targetType, studentId, scope.userId),
    include: includeAssignment,
  });
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'STUDENT_FEE_ASSIGNMENT', entityId: item.id, action: 'UPDATE', beforeState: existing, afterState: item });
...
export const deleteFeeAssignment = async (req: Request, res: Response) => {
  ...
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'STUDENT_FEE_ASSIGNMENT', entityId: item.id, action: 'DELETE', beforeState: existing, afterState: item });
  res.status(200).json(item);
};
...
export const activateFeeAssignment = async (req: Request, res: Response) => {
  ...
  if (duplicate) throw new HttpError(409, 'Duplicate active assignment exists for the same target, structure, and academic session');
  const item = await FeeAssignmentRepository.studentFeeAssignment.update({ where: { id: existing.id }, data: { status: 'ACTIVE', updatedById: scope.userId }, include: includeAssignment });
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'STUDENT_FEE_ASSIGNMENT', entityId: item.id, action: 'ACTIVE', beforeState: existing, afterState: item });
  res.status(200).json(item);
};
...
export const deactivateFeeAssignment = async (req: Request, res: Response) => {
  ...
  const existing = await FeeAssignmentRepository.studentFeeAssignment.findFirst({ where: { id: uuidParam(req), ...tenantScopeOnly(scope), deletedAt: null } });
  if (!existing) throw new HttpError(404, 'Fee assignment not found');
  const item = await FeeAssignmentRepository.studentFeeAssignment.update({ where: { id: existing.id }, data: { status: 'INACTIVE', updatedById: scope.userId }, include: includeAssignment });
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'STUDENT_FEE_ASSIGNMENT', entityId: item.id, action: 'INACTIVE', beforeState: existing, afterState: item });
  res.status(200).json(item);
};
