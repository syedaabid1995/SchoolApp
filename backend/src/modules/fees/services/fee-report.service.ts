...
export const feeReportQuerySchema = z.object({
  ...
  feeGroupId: uuidSchema.optional(),
  feeMasterId: uuidSchema.optional(),
...
export const buildFeeReport = async (scope: FeeReportScope, query: FeeReportQuery) => {
  ...
  const invoiceWhere: Prisma.FeeInvoiceWhereInput = {
    ...
    ...(query.feeGroupId ? { feeGroupId: query.feeGroupId } : {}),
    ...(query.feeMasterId ? { items: { some: { feeMasterId: query.feeMasterId } } } : {}),
  ...
  const discountWhere: Prisma.FeeDiscountWhereInput = {
    ...
    ...(query.feeTypeId ? { feeTypeId: query.feeTypeId } : {}),
    ...(query.feeGroupId ? { feeGroupId: query.feeGroupId } : {}),
    ...(query.feeMasterId ? { feeMasterId: query.feeMasterId } : {}),
  ...
  const [payments, invoices, discounts, fines, fineLedgers, receipts, ledgers] = await Promise.all([
    ...
    FeeReportingRepository.feeInvoice.findMany({
      ...
      include: {
        ...
        items: { select: { feeMasterId: true, feeMaster: { select: { name: true, code: true } } } },
      },
    ...
    FeeReportingRepository.feeDiscount.findMany({
      ...
      include: {
        ...
        feeType: { select: { name: true } },
        feeGroup: { select: { name: true } },
        feeMaster: { select: { name: true, code: true } },
      },
  ...
  return {
    ...
    filters: {
      ...
      feeGroupId: query.feeGroupId ?? null,
      feeMasterId: query.feeMasterId ?? null,
