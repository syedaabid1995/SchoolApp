...
import { z } from 'zod';
import { FeeStructureRepository } from '../../repositories/structure.repository';
...
const resolveAcademicSessionId = async (schoolId: string, requested?: string | null) => {
  if (requested) {
    const found = await FeeStructureRepository.academicYear.findFirst({ where: { id: requested, schoolId }, select: { id: true } });
    if (!found) throw new HttpError(404, 'Academic session not found');
    return requested;
  }

  const active = await FeeStructureRepository.academicYear.findFirst({
  ...
  const latest = await FeeStructureRepository.academicYear.findFirst({
    where: { schoolId },
    orderBy: { startDate: 'desc' },
    select: { id: true },
  });
...
const ensureFeeDefaults = async (schoolId: string, academicSessionId: string) => {
  const [particularCount, typeCount] = await Promise.all([
    FeeStructureRepository.feeParticular.count({ where: { schoolId, academicSessionId } }),
    FeeStructureRepository.feeType.count({ where: { schoolId, academicSessionId } }),
  ]);

  if (!particularCount) {
    await FeeStructureRepository.feeParticular.createMany({
      skipDuplicates: true,
  ...
  if (!typeCount) {
    await FeeStructureRepository.feeType.createMany({
      skipDuplicates: true,
...
const assertClass = async (schoolId: string, classId: string) => {
  const found = await FeeStructureRepository.class.findFirst({ where: { id: classId, schoolId }, select: { id: true } });
  if (!found) throw new HttpError(404, 'Class not found');
};

const assertSection = async (schoolId: string, sectionId?: string | null) => {
  if (!sectionId) return;
  const found = await FeeStructureRepository.section.findFirst({ where: { id: sectionId, schoolId }, select: { id: true } });
...
const assertStudent = async (schoolId: string, studentId: string, academicSessionId?: string) => {
  const found = await FeeStructureRepository.student.findFirst({
...
const assertFeeType = async (schoolId: string, academicSessionId: string, feeTypeId: string) => {
  const found = await FeeStructureRepository.feeType.findFirst({
...
const assertFeeGroup = async (schoolId: string, academicSessionId: string, feeGroupId: string) => {
  const found = await FeeStructureRepository.feeGroup.findFirst({
...
const assertFeeMaster = async (schoolId: string, academicSessionId: string, feeMasterId: string) => {
  const found = await FeeStructureRepository.feeMaster.findFirst({
...
const assertStudentCategory = async (schoolId: string, categoryId: string) => {
  const found = await FeeStructureRepository.studentCategory.findFirst({ where: { id: categoryId, schoolId }, select: { id: true } });
  if (!found) throw new HttpError(404, 'Student category not found');
};

const assertStudentGroup = async (schoolId: string, groupId: string) => {
  const found = await FeeStructureRepository.studentGroup.findFirst({ where: { id: groupId, schoolId }, select: { id: true } });
...
const assertTransportRoute = async (schoolId: string, transportRouteId: string) => {
  const found = await FeeStructureRepository.transportRoute.findFirst({ where: { id: transportRouteId, schoolId }, select: { id: true } });
...
const assertParticulars = async (schoolId: string, academicSessionId: string, ids: string[]) => {
  ...
  const found = await FeeStructureRepository.feeParticular.findMany({
...
const assertNoDuplicateActiveDiscount = async (scope: FeeTenantScope, payload: NormalizedDiscountPayload, excludeId?: string) => {
  ...
  const duplicate = await FeeStructureRepository.feeDiscount.findFirst({
    where: {
...
const assertDiscountNotLockedByPayment = async (scope: FeeTenantScope, discount: FeeDiscount) => {
  const paidCount = await FeeStructureRepository.feeInvoice.count({ where: buildDiscountInvoiceWhere(scope, discount) });
  if (paidCount > 0) {
...
const assertDiscountDoesNotExceedCurrentPayable = async (scope: FeeTenantScope, payload: NormalizedDiscountPayload) => {
  ...
  const payable = toDecimal(
    (
      await FeeStructureRepository.feeInvoice.aggregate({
        where: {
          ...tenantScopeOnly(scope),
          studentId: payload.studentId,
          deletedAt: null,
...
const assertAssignmentReferences = async (scope: FeeTenantScope, payload: z.infer<typeof assignmentSchema>, targetType: FeeAssignmentTarget) => {
  ...
  if (targetType === 'STUDENT') {
    ...
    const found = await FeeStructureRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: ids } }), select: { id: true } });
...
const resolveAssignmentStudents = async (
  ...
  if (targetType === 'STUDENT') {
    return FeeStructureRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: studentIds } }), include, orderBy: { fullName: 'asc' } });
  }
  if (targetType === 'TRANSPORT_ROUTE') {
    const rows = await FeeStructureRepository.studentTransportAssignment.findMany({
      where: { schoolId: scope.schoolId, routeId: payload.transportRouteId ?? '', active: true },
      select: { studentId: true },
    });
    return FeeStructureRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: rows.map((row) => row.studentId) } }), include, orderBy: { fullName: 'asc' } });
  }
  return FeeStructureRepository.student.findMany({
...
export const getFeeMetadata = async (req: Request, res: Response) => {
  ...
  const [academicSessions, classes, sections, students, studentGroups, studentCategories, particulars, feeTypes, structures, transportRoutes] = await Promise.all([
    FeeStructureRepository.academicYear.findMany({ where: { schoolId: scope.schoolId }, orderBy: { startDate: 'desc' }, select: { id: true, name: true, isActive: true } }),
    FeeStructureRepository.class.findMany({ where: { schoolId: scope.schoolId }, orderBy: { name: 'asc' }, select: { id: true, name: true } }),
    FeeStructureRepository.section.findMany({ where: { schoolId: scope.schoolId }, orderBy: { name: 'asc' }, select: { id: true, name: true, classId: true } }),
    FeeStructureRepository.student.findMany({
    ...
    FeeStructureRepository.studentGroup.findMany({ where: { schoolId: scope.schoolId }, orderBy: { name: 'asc' }, select: { id: true, name: true } }),
    FeeStructureRepository.studentCategory.findMany({ where: { schoolId: scope.schoolId }, orderBy: { name: 'asc' }, select: { id: true, name: true } }),
    FeeStructureRepository.feeParticular.findMany({ where: { ...tenantScopeOnly(scope), deletedAt: null }, orderBy: [{ sortOrder: 'asc' }, { name: 'asc' }] }),
    FeeStructureRepository.feeType.findMany({ where: { ...tenantScopeOnly(scope), deletedAt: null }, orderBy: [{ sortOrder: 'asc' }, { name: 'asc' }] }),
    FeeStructureRepository.feeStructure.findMany({ where: { ...tenantScopeOnly(scope), deletedAt: null }, include: includeStructure, orderBy: { createdAt: 'desc' }, take: 100 }),
    FeeStructureRepository.transportRoute.findMany({ where: { schoolId: scope.schoolId }, orderBy: { title: 'asc' }, select: { id: true, title: true, fare: true } }),
  ]);
...
const assertUniqueFeeParticularName = async (scope: FeeTenantScope, normalizedName: string, excludeId?: string) => {
  const duplicate = await FeeStructureRepository.feeParticular.findFirst({
    where: {
      ...tenantScopeOnly(scope),
      normalizedName,
      ...(excludeId ? { id: { not: excludeId } } : {}),
...
export const listFeeParticulars = async (req: Request, res: Response) => {
  ...
  const [items, total] = await Promise.all([
    FeeStructureRepository.feeParticular.findMany({ where, orderBy: [{ sortOrder: 'asc' }, { name: 'asc' }], skip, take: limit }),
    FeeStructureRepository.feeParticular.count({ where }),
  ]);
...
export const createFeeParticular = async (req: Request, res: Response) => {
  ...
  try {
    const item = await FeeStructureRepository.feeParticular.create({
      data: {
        ...tenantScopeOnly(scope),
        name,
        normalizedName,
    ...
    await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_PARTICULAR', entityId: item.id, action: 'CREATE', afterState: item });
...
export const updateFeeParticular = async (req: Request, res: Response) => {
  ...
  const scope = await resolveScope(req, payload);
  const existing = await FeeStructureRepository.feeParticular.findFirst({ where: { id: uuidParam(req), ...tenantScopeOnly(scope), deletedAt: null } });
  ...
  try {
    const item = await FeeStructureRepository.feeParticular.update({
    ...
    await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_PARTICULAR', entityId: item.id, action: 'UPDATE', beforeState: existing, afterState: item });
...
export const deleteFeeParticular = async (req: Request, res: Response) => {
  ...
  const existing = await FeeStructureRepository.feeParticular.findFirst({ where: { id: uuidParam(req), ...tenantScopeOnly(scope), deletedAt: null } });
  if (!existing) throw new HttpError(404, 'Fee particular not found');
  const usage = await FeeStructureRepository.feeStructureItem.count({
    where: {
  ...
  const item = await FeeStructureRepository.feeParticular.update({ where: { id: existing.id }, data: { deletedAt: new Date(), status: 'INACTIVE' } });
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_PARTICULAR', entityId: item.id, action: 'DELETE', beforeState: existing, afterState: item });
  res.status(204).send();
};
...
const assertUniqueFeeTypeName = async (scope: FeeTenantScope, normalizedName: string, excludeId?: string) => {
  const duplicate = await FeeStructureRepository.feeType.findFirst({
    where: {
      ...tenantScopeOnly(scope),
      normalizedName,
      ...(excludeId ? { id: { not: excludeId } } : {}),
...
export const listFeeTypes = async (req: Request, res: Response) => {
  const scope = await resolveScope(req);
  await ensureFeeDefaults(scope.schoolId, scope.academicSessionId);
  const items = await FeeStructureRepository.feeType.findMany({ where: { ...tenantScopeOnly(scope), deletedAt: null }, orderBy: [{ sortOrder: 'asc' }, { name: 'asc' }] });
...
export const createFeeType = async (req: Request, res: Response) => {
  ...
  try {
    const item = await FeeStructureRepository.feeType.create({
      data: {
        ...tenantScopeOnly(scope),
        name,
        normalizedName,
    ...
    await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_TYPE', entityId: item.id, action: 'CREATE', afterState: item });
    res.status(201).json(item);
  } catch (err) {
...
export const updateFeeType = async (req: Request, res: Response) => {
  ...
  const scope = await resolveScope(req, payload);
  const existing = await FeeStructureRepository.feeType.findFirst({ where: { id: uuidParam(req), ...tenantScopeOnly(scope), deletedAt: null } });
  ...
  try {
    const item = await FeeStructureRepository.feeType.update({
      where: { id: existing.id },
      data: {
        name: payload.name === undefined ? undefined : normalizeText(payload.name),
        normalizedName,
    ...
    await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_TYPE', entityId: item.id, action: 'UPDATE', beforeState: existing, afterState: item });
    res.status(200).json(item);
  } catch (err) {
    if (uniqueTargetIncludes(err, 'normalized_name')) throw new HttpError(409, 'Fee type name already exists for this session');
    handleUniqueError(err, 'Fee type code already exists for this session');
...
export const deleteFeeType = async (req: Request, res: Response) => {
  ...
  const existing = await FeeStructureRepository.feeType.findFirst({ where: { id: uuidParam(req), ...tenantScopeOnly(scope), deletedAt: null } });
  if (!existing) throw new HttpError(404, 'Fee type not found');
  const usage = await FeeStructureRepository.feeStructure.count({ where: { feeTypeId: existing.id, deletedAt: null } });
  if (usage) throw new HttpError(409, 'Cannot delete fee type while structures use it');
  const item = await FeeStructureRepository.feeType.update({ where: { id: existing.id }, data: { deletedAt: new Date(), status: 'INACTIVE' } });
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_TYPE', entityId: item.id, action: 'DELETE', beforeState: existing, afterState: item });
  res.status(204).send();
};
...
export const listFeeStructures = async (req: Request, res: Response) => {
  ...
  const [items, total] = await Promise.all([
    FeeStructureRepository.feeStructure.findMany({ where, include: includeStructure, orderBy: { createdAt: 'desc' }, skip, take: limit }),
    FeeStructureRepository.feeStructure.count({ where }),
  ]);
  res.status(200).json({ items, pagination: { page, limit, total, totalPages: Math.max(1, Math.ceil(total / limit)) } });
};
...
export const createFeeStructure = async (req: Request, res: Response) => {
  ...
  try {
    const item = await FeeStructureRepository.feeStructure.create({
      data: {
        ...tenantScopeOnly(scope),
        classId: payload.classId,
        sectionId: payload.sectionId ?? null,
    ...
    await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_STRUCTURE', entityId: item.id, action: 'CREATE', afterState: item });
    res.status(201).json(item);
  } catch (err) {
...
export const updateFeeStructure = async (req: Request, res: Response) => {
  ...
  const scope = await resolveScope(req, payload);
  const existing = await FeeStructureRepository.feeStructure.findFirst({ where: { id: uuidParam(req), ...tenantScopeOnly(scope), deletedAt: null }, include: includeStructure });
  ...
  try {
    const item = await FeeStructureRepository.$transaction(async (tx) => {
      if (payload.items) {
    ...
    await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_STRUCTURE', entityId: item.id, action: 'UPDATE', beforeState: existing, afterState: item });
    res.status(200).json(item);
  } catch (err) {
    handleUniqueError(err, 'A fee structure already exists for this class, section, session, and fee type');
  }
...
export const deleteFeeStructure = async (req: Request, res: Response) => {
  const scope = await resolveScope(req);
  const existing = await FeeStructureRepository.feeStructure.findFirst({ where: { id: uuidParam(req), ...tenantScopeOnly(scope), deletedAt: null }, include: { _count: { select: { assignments: true, invoices: true } } } });
  if (!existing) throw new HttpError(404, 'Fee structure not found');
  ...
  const item = await FeeStructureRepository.feeStructure.update({ where: { id: existing.id }, data: { deletedAt: new Date(), status: 'INACTIVE' } });
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_STRUCTURE', entityId: item.id, action: 'DELETE', beforeState: existing, afterState: item });
  res.status(204).send();
};
...
export const duplicateFeeStructure = async (req: Request, res: Response) => {
  ...
  const scope = await resolveScope(req, payload);
  const source = await FeeStructureRepository.feeStructure.findFirst({ where: { id: uuidParam(req), ...tenantScopeOnly(scope), deletedAt: null }, include: includeStructure });
