...
import { z } from 'zod';
import { FeeDiscountRepository } from '../../repositories/discount.repository';
...
const resolveAcademicSessionId = async (schoolId: string, requested?: string | null) => {
  if (requested) {
    const found = await FeeDiscountRepository.academicYear.findFirst({ where: { id: requested, schoolId }, select: { id: true } });
    if (!found) throw new HttpError(404, 'Academic session not found');
    return requested;
  }

  const active = await FeeDiscountRepository.academicYear.findFirst({
  ...
  const latest = await FeeDiscountRepository.academicYear.findFirst({
    where: { schoolId },
    orderBy: { startDate: 'desc' },
    select: { id: true },
  });
...
const ensureFeeDefaults = async (schoolId: string, academicSessionId: string) => {
  const [particularCount, typeCount] = await Promise.all([
    FeeDiscountRepository.feeParticular.count({ where: { schoolId, academicSessionId } }),
    FeeDiscountRepository.feeType.count({ where: { schoolId, academicSessionId } }),
  ]);

  if (!particularCount) {
    await FeeDiscountRepository.feeParticular.createMany({
      skipDuplicates: true,
  ...
  if (!typeCount) {
    await FeeDiscountRepository.feeType.createMany({
      skipDuplicates: true,
...
const assertClass = async (schoolId: string, classId: string) => {
  const found = await FeeDiscountRepository.class.findFirst({ where: { id: classId, schoolId }, select: { id: true } });
  if (!found) throw new HttpError(404, 'Class not found');
};

const assertSection = async (schoolId: string, sectionId?: string | null) => {
  if (!sectionId) return;
  const found = await FeeDiscountRepository.section.findFirst({ where: { id: sectionId, schoolId }, select: { id: true } });
...
const assertStudent = async (schoolId: string, studentId: string, academicSessionId?: string) => {
  const found = await FeeDiscountRepository.student.findFirst({
...
const assertFeeType = async (schoolId: string, academicSessionId: string, feeTypeId: string) => {
  const found = await FeeDiscountRepository.feeType.findFirst({
...
const assertFeeGroup = async (schoolId: string, academicSessionId: string, feeGroupId: string) => {
  const found = await FeeDiscountRepository.feeGroup.findFirst({
...
const assertFeeMaster = async (schoolId: string, academicSessionId: string, feeMasterId: string) => {
  const found = await FeeDiscountRepository.feeMaster.findFirst({
...
const assertStudentCategory = async (schoolId: string, categoryId: string) => {
  const found = await FeeDiscountRepository.studentCategory.findFirst({ where: { id: categoryId, schoolId }, select: { id: true } });
  if (!found) throw new HttpError(404, 'Student category not found');
};

const assertStudentGroup = async (schoolId: string, groupId: string) => {
  const found = await FeeDiscountRepository.studentGroup.findFirst({ where: { id: groupId, schoolId }, select: { id: true } });
...
const assertTransportRoute = async (schoolId: string, transportRouteId: string) => {
  const found = await FeeDiscountRepository.transportRoute.findFirst({ where: { id: transportRouteId, schoolId }, select: { id: true } });
...
const assertParticulars = async (schoolId: string, academicSessionId: string, ids: string[]) => {
  ...
  const found = await FeeDiscountRepository.feeParticular.findMany({
...
const normalizeDiscountPayload = (payload: z.infer<typeof discountSchema>, fallbackStatus: FeeDiscountStatus = 'PENDING_APPROVAL'): NormalizedDiscountPayload => {
  ...
  const normalized: NormalizedDiscountPayload = {
    ...
    code: nullableText(payload.code),
...
const assertNoDuplicateActiveDiscount = async (scope: FeeTenantScope, payload: NormalizedDiscountPayload, excludeId?: string) => {
  ...
  const duplicate = await FeeDiscountRepository.feeDiscount.findFirst({
    where: {
...
const assertDiscountNotLockedByPayment = async (scope: FeeTenantScope, discount: FeeDiscount) => {
  const paidCount = await FeeDiscountRepository.feeInvoice.count({ where: buildDiscountInvoiceWhere(scope, discount) });
  if (paidCount > 0) {
...
const assertDiscountDoesNotExceedCurrentPayable = async (scope: FeeTenantScope, payload: NormalizedDiscountPayload) => {
  ...
  const payable = toDecimal(
    (
      await FeeDiscountRepository.feeInvoice.aggregate({
        where: {
          ...tenantScopeOnly(scope),
          studentId: payload.studentId,
          deletedAt: null,
...
const assertAssignmentReferences = async (scope: FeeTenantScope, payload: z.infer<typeof assignmentSchema>, targetType: FeeAssignmentTarget) => {
  ...
  if (targetType === 'STUDENT') {
    ...
    const found = await FeeDiscountRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: ids } }), select: { id: true } });
...
const resolveAssignmentStudents = async (
  ...
  if (targetType === 'STUDENT') {
    return FeeDiscountRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: studentIds } }), include, orderBy: { fullName: 'asc' } });
  }
  if (targetType === 'TRANSPORT_ROUTE') {
    const rows = await FeeDiscountRepository.studentTransportAssignment.findMany({
      where: { schoolId: scope.schoolId, routeId: payload.transportRouteId ?? '', active: true },
      select: { studentId: true },
    });
    return FeeDiscountRepository.student.findMany({ where: activeStudentWhere(scope, { id: { in: rows.map((row) => row.studentId) } }), include, orderBy: { fullName: 'asc' } });
  }
  return FeeDiscountRepository.student.findMany({
...
export const listFeeDiscounts = async (req: Request, res: Response) => {
  ...
  const where: Prisma.FeeDiscountWhereInput = {
    ...
    ...(typeof req.query.feeGroupId === 'string' ? { feeGroupId: req.query.feeGroupId } : {}),
    ...(typeof req.query.feeMasterId === 'string' ? { feeMasterId: req.query.feeMasterId } : {}),
    ...(search
      ? {
          OR: [
            ...
            { feeGroup: { is: { name: { contains: search, mode: 'insensitive' } } } },
            { feeMaster: { is: { name: { contains: search, mode: 'insensitive' } } } },
            { feeMaster: { is: { code: { contains: search, mode: 'insensitive' } } } },
          ],
        }
      : {}),
    ...(dateFrom || dateTo
  ...
  const [items, total] = await FeeDiscountRepository.$transaction([
    FeeDiscountRepository.feeDiscount.findMany({
      where,
      include: {
        ...
        feeMaster: { select: { id: true, name: true, code: true } },
      },
      orderBy: { [sortBy]: sortOrder },
      skip,
      take: limit,
    }),
    FeeDiscountRepository.feeDiscount.count({ where }),
  ]);
  res.status(200).json({
    items,
    data: items,
...
export const createFeeDiscount = async (req: Request, res: Response) => {
  ...
  const item = await FeeDiscountRepository.$transaction(async (tx) => {
  ...
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_DISCOUNT', entityId: item.id, action: 'CREATE', afterState: item });
...
export const updateFeeDiscount = async (req: Request, res: Response) => {
  ...
  const tenantScope = { schoolId: scope.schoolId, academicSessionId: scope.academicSessionId };
  const existing = await FeeDiscountRepository.feeDiscount.findFirst({ where: { id: uuidParam(req), ...tenantScope, deletedAt: null } });
  ...
  const item = await FeeDiscountRepository.$transaction(async (tx) => {
    const discount = await tx.feeDiscount.update({
      ...
      data: {
        ...
        feeGroupId: merged.feeGroupId,
        feeMasterId: merged.feeMasterId,
  ...
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_DISCOUNT', entityId: item.id, action: 'UPDATE', beforeState: existing, afterState: item });
...
export const deleteFeeDiscount = async (req: Request, res: Response) => {
  ...
  if (!existing) throw new HttpError(404, 'Discount not found');
  await assertDiscountNotLockedByPayment(tenantScope, existing);
  const item = await FeeDiscountRepository.feeDiscount.update({
    where: { id: existing.id },
    data: { deletedAt: new Date(), approvalStatus: 'INACTIVE', updatedById: scope.userId },
  });
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_DISCOUNT', entityId: item.id, action: 'DELETE', beforeState: existing, afterState: item });
  res.status(200).json(item);
};
...
const reviewFeeDiscount = async (req: Request, status: Extract<FeeDiscountStatus, 'APPROVED' | 'ACTIVE' | 'REJECTED'>) => {
  ...
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_DISCOUNT', entityId: item.id, action: status, beforeState: existing, afterState: item });
  return item;
};
...
export const deactivateFeeDiscount = async (req: Request, res: Response) => {
  ...
  await FeeAuditService.record(req, { schoolId: scope.schoolId, entityType: 'FEE_DISCOUNT', entityId: item.id, action: 'INACTIVE', beforeState: existing, afterState: item });
  res.status(200).json(item);
};
