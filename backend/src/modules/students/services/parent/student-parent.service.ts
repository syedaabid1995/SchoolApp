...
import { StudentParentRepository } from '../../repositories/parent.repository';
...
const ensureAcademicScope = async (schoolId: string, payload: { academicSessionId?: string | null; classId?: string | null; sectionId?: string | null }) => {
  if (payload.academicSessionId) {
    const year = await StudentParentRepository.academicYear.findFirst({ where: { id: payload.academicSessionId, schoolId }, select: { id: true } });
    if (!year) throw new HttpError(404, 'Academic session not found');
  }
  if (payload.classId) {
    const cls = await StudentParentRepository.class.findFirst({ where: { id: payload.classId, schoolId }, select: { id: true } });
    if (!cls) throw new HttpError(404, 'Class not found');
  }
  if (payload.sectionId) {
    const section = await StudentParentRepository.section.findFirst({ where: { id: payload.sectionId, schoolId }, select: { id: true } });
    if (!section) throw new HttpError(404, 'Section not found');
  }
  if (payload.classId && payload.sectionId) {
    const link = await StudentParentRepository.classSection.findFirst({ where: { schoolId, classId: payload.classId, sectionId: payload.sectionId }, select: { id: true } });
...
const ensureRollIsUnique = async (
  ...
  const existing = await StudentParentRepository.studentEnrollment.findFirst({
    where: {
      schoolId,
...
export const linkParent = async (req: Request, res: Response) => {
  ...
  const student = await StudentParentRepository.student.findFirst({
    where: { id, schoolId },
    select: { id: true },
  });
  ...
  const parent = await StudentParentRepository.parentProfile.findFirst({
...
export const unlinkParent = async (req: Request, res: Response) => {
  ...
  const existing = await StudentParentRepository.studentParent.findFirst({
  ...
  await StudentParentRepository.studentParent.delete({
