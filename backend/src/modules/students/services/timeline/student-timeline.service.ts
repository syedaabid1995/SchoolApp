...
import { StudentTimelineRepository } from '../../repositories/timeline.repository';
...
const ensureAcademicScope = async (schoolId: string, payload: { academicSessionId?: string | null; classId?: string | null; sectionId?: string | null }) => {
  if (payload.academicSessionId) {
    const year = await StudentTimelineRepository.academicYear.findFirst({ where: { id: payload.academicSessionId, schoolId }, select: { id: true } });
    if (!year) throw new HttpError(404, 'Academic session not found');
  }
  if (payload.classId) {
    const cls = await StudentTimelineRepository.class.findFirst({ where: { id: payload.classId, schoolId }, select: { id: true } });
    if (!cls) throw new HttpError(404, 'Class not found');
  }
  if (payload.sectionId) {
    const section = await StudentTimelineRepository.section.findFirst({ where: { id: payload.sectionId, schoolId }, select: { id: true } });
    if (!section) throw new HttpError(404, 'Section not found');
  }
  if (payload.classId && payload.sectionId) {
    const link = await StudentTimelineRepository.classSection.findFirst({ where: { schoolId, classId: payload.classId, sectionId: payload.sectionId }, select: { id: true } });
...
const ensureRollIsUnique = async (
  ...
  const existing = await StudentTimelineRepository.studentEnrollment.findFirst({
    where: {
      schoolId,
...
export const addStudentTimeline = async (req: Request, res: Response) => {
  ...
  const { id } = req.params;
  const student = await StudentTimelineRepository.student.findFirst({ where: { id, schoolId }, select: { id: true } });
  if (!student) throw new HttpError(404, 'Student not found');
  const item = await StudentTimelineRepository.studentTimeline.create({
    data: {
      schoolId,
      studentId: id,
...
export const deleteStudentTimeline = async (req: Request, res: Response) => {
  ...
  const item = await StudentTimelineRepository.studentTimeline.findFirst({ where: { id: timelineId, studentId: id, schoolId } });
  if (!item) throw new HttpError(404, 'Timeline item not found');
  await StudentTimelineRepository.studentTimeline.delete({ where: { id: timelineId } });
  await AuditLogService.record(req, {
    schoolId,
    entityType: 'STUDENT_TIMELINE',
    entityId: timelineId,
