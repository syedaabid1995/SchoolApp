...
const parseOptions = (): Options => {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--apply') options.apply = true;
      ...
      console.log('Usage: tsx scripts/migrate-parent-profile-sensitive-encryption.ts [--dry-run] [--apply] [--school-id <id>] [--limit <n>]');
      process.exit(0);
    }
  }
...
const storageFields = Array.from(new Set([
  ...PARENT_PROFILE_ENCRYPT_FIELDS,
  ...PARENT_PROFILE_LOOKUP_HASH_FIELDS.map((field) => `${field}Hash` as const),
]));

const main = async () => {
  const options = parseOptions();
  if (env.SENSITIVE_FIELD_ENCRYPTION_ENABLED && !env.SENSITIVE_FIELD_ENCRYPTION_KEY) {
    throw new Error('SENSITIVE_FIELD_ENCRYPTION_KEY is required to migrate parent profile sensitive encryption');
  }
  ...
  for (const rawRow of rows) {
    const decrypted = decryptParentProfileSensitiveFields(rawRow);
    const encrypted = encryptParentProfileSensitiveFields(decrypted);
    ...
    for (const field of storageFields) {
      ...
      if (
        (PARENT_PROFILE_ENCRYPT_FIELDS as readonly string[]).includes(field) &&
...
main()
  .catch((error) => {
    console.error(JSON.stringify({
      ok: false,
      message: error instanceof Error ? error.message : 'Parent profile sensitive encryption migration failed.',
    }, null, 2));
    process.exitCode = 1;
  })
  .finally(async () => {
